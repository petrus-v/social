...
class MailComposeMessage(models.TransientModel):
    ...
    @api.model
    def _match_attachement(self, re_pattern, model, res_ids):
    ...
    def _compute_object_attachment_ids(self):
        for composer in self:
            if (
                ...
                composer.object_attachment_ids |= composer._match_attachement(
    ...
    def get_mail_values(self, res_ids):
        ...
        if (
            ...
            attachment_per_res_id = tools.groupby(
                self._match_attachement(pattern, self.model, res_ids),
